-- Fix: the occupancy trend rendered "Invalid Date" at both ends.
--
-- generate_series(date, date, interval) returns timestamptz, not date, so each
-- trend entry arrived as "2026-09-22T00:00:00+00:00" rather than "2026-09-22".
-- The client treats stay dates as plain YYYY-MM-DD strings (a calendar day is
-- not an instant — see booking-rules.ts), so parsing produced Invalid Date.
--
-- It also broke the "today" comparison silently: point.date === today never
-- matched, so today's bar was never highlighted and the past/future split did
-- not render.
--
-- Casting to ::date here rather than reformatting in the client, because the
-- wire format is the contract and every other date the API returns is a plain
-- date already.
  ...
  select jsonb_build_object(
    ...
    'trend', coalesce((
      select jsonb_agg(jsonb_build_object(
               -- ::date, so this is a calendar day and not an instant.
               'date', d.day::date,
               'occupied', (
